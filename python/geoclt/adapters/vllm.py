...
class VllmAdapter(AdapterBase):
    ...
    def _unavailable(self) -> RuntimeError:
        return RuntimeError("vLLM runtime is not installed or configured in this environment")
    ...
    def infer_logits_hash(self, prompt: str) -> str:
        return sha256(f"vllm:{self.model_id}:{prompt}".encode("utf-8")).hexdigest()


def attach_vllm_hooks(engine, layers=None):
    selected = list(layers or [])
    return {"engine": type(engine).__name__, "layers": selected, "mode": "passive"}
